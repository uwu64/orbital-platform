...
void init_i2c2() {
	...
	// flush bus 
	GPIOF->MODER = (1 << 0*2) | (1 << 1*2); // gpio to output 
	for (int i = 0; i < 16; i++) { // send clock pulses 
		GPIOF->BSRR = 1 << (1+16); 
		nop(current_core_freq_MHz*2);
		GPIOF->BSRR = 1 << 1; 
		nop(current_core_freq_MHz*2);
	}
	// GPIOF->BSRR = 1 << (1+16) | 1 << (0+16); // bit bang to issue stop condition 
	// nop(current_core_freq_MHz*2);
	// GPIOF->BSRR = 1 << 1; 
	// nop(current_core_freq_MHz*2);
	// GPIOF->BSRR = 1 << 0; 
	...
	GPIOF->MODER = (2 << 0*2) | (2 << 1*2); // gpio to AF mode 
	// I2C2->CR1 = 0; // reset peripheral (no need, start at 0)
	...
	I2C2->CR1 = 1; // peripheral enable 
	//I2C2->CR1 = 1 | 0xf << 8; // peripheral enable and digital noise filter 
}
...
//	i2c2_tx(0xD4, &lol, 1, &lmao, 1);
	...
	int lolc[10] = {0x07};
	i2c2_tx(0x0D<<1, &lolc, 1, &lmao, 0);
	i2c2_tx(0x0D<<1, &lolc, 0, &lmao, 1);
